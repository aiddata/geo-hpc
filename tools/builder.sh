...
# clone tmp geo-hpc for init scripts
git clone -b "$branch" https://github.com/itpir/geo-hpc tmp/geo-hpc
ln -sfn tmp/geo-hpc "$src"/geo-hpc

# run load_repos.sh
bash "$src"/tmp/geo-hpc/tools/load_repos.sh "$branch" #2>&1 | tee "$src"/log/load_repos/$(date +%s).load_repos.log


# setup crons using manage_crons.sh script
bash "$src"/tmp/geo-hpc/tools/manage_crons.sh "$branch" init
...
python "$src"/tmp/geo-hpc/tools/add_config_to_mongo.py "$branch"



# clean up tmp geo-hpc
rm -rf "$src"/tmp/geo-hpc
