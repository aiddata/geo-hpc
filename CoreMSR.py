...
class CoreMSR():
    ...
    Attributes:
        ...
        is_geocoded (str): boolean field (from csv files) identifying if project is geocoded
        ...
        adm0 : prepared shapely shape representing coarsest spatial unit
    ...
    def set_adm0(self, pshape):
        """Set value of adm0 attribute

        Args:
            pshape: prepared shapely shape

        Will exit script if pshape is not a prepared shapely shape
        """
        if isinstance(val, type(prep(Point(0,0)))):
            self.adm0 = val
        else:
    ...
    def merge_data(self, path, merge_id, field_ids, only_geo):
    ...
    def geom_to_grid_colrows(geom, step, rounded=True, no_multi=False):
        ...
        Args:
            geom (shape): geometry to be used (polygon / multipolygon)
        ...
        Returns:
            tuple of lists for columns (longitude) and rows (latitude) of grid 
            (columns, rows)
        """
