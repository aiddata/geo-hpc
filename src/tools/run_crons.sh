...
case $cron in
    ...
    "build_db_updates_job")  mkdir -p "$src"/log/db_updates
                            bash "$src"/asdf/src/tasks/build_db_updates_job.sh "$branch" "$timestamp" 2>&1 | tee 1>>"$src"/log/db_updates/"$timestamp".db_updates.log
                            exit 0;;
