...
for i in latest_releases:
    ...
    for filter_fields in dataset_info[ix]['iter']:
        ...
        mongo_core_doc.update(mongo_update_doc)


        # add to msr tracker if hash does not exist
        exists = c_msr.update_one({'hash': mongo_core_doc['hash']},
                                  {'$setOnInsert': mongo_core_doc},
                                  upsert=True)
        ...
        if exists.upserted_id != None:
            ...
            c_msr.update_one({
                'hash': mongo_core_doc['hash'],
                'status': 0,
                '$or': [
                    {'count': {'$ne': filter_count}},
                    {'percentage': {'$ne': filter_percentage}}
                ]
            },
    ...
    print str(update_count) + ' additional items updated.'
