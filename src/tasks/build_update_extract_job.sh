...
#   timestamp
#
# builds job that updates extract queue
# job runs the bash script update_extract_script.sh
#
...
if echo "$qstat" | grep -q 'ax-upe-'"$branch"; then
    ...
    job_dir="$src"/log/update_extract #/jobs
    mkdir -p $job_dir
...
#PBS -o $src/log/update_extract/jobs/$timestamp.$jobtime.update_extract.job
...
EOF
    ...
    # cd "$src"/log/update_extract/jobs
