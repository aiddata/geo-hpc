    ...
    for match in matches:
        ...
        if bnd["scale"] == "global":
            ...
            if len(clean_values) > len(values)*valid_sample_thresh:# and len(distinct_values) > 1:
                ...
                print('\t\t\t\t # All values: {}'.format(len(values)))
                print('\t\t\t\t # Clean values: {}'.format(len(clean_values)))
                print('\t\t\t\t # Thresh size (thresh={}): {}'.format(valid_sample_thresh, len(values)*valid_sample_thresh))
                print('\t\t\t\t # Distinct values: {}'.format(len(distinct_values)))
